...
def evaluate_model(models_dir,modelfile,xtest,ytest,test_snr_sequence):
	...
	plt.plot(SNR,val_acc,label = 'validation_accuracy')
	plt.title('Accuracy vs SNRs')
	plt.xlabel('Accuracy')
	plt.xlabel('SNRs')
	plt.legend()
	plt.grid(True)
	plt.savefig(models_dir + 'fig_val_acc.png')